...
## 出力形式
以下の2つの訳を出力すること（解説・補足・注釈は不要）:
- 訳A（直訳寄り）: 原文の構造に忠実な英訳
- 訳B（意訳寄り）: 英語として自然な構造に再編した英訳"""
...
あなたは翻訳の専門家です。
4種のLLM（Claude, Gemini, ChatGPT, Grok）が同じ日本語文をそれぞれ2案ずつ英訳しました。
これらを分析し、指定された形式で統合結果を作成してください。
各LLMの良い表現を積極的に取り込み、採用した表現には出典LLMを括弧で注記してください。
さらに、各LLMの英訳を踏まえた上で、より良い表現がないか独自に検討し、改善案があれば提示してください。"""
...
また、各LLMの英訳にない表現でもより適切なものがあれば、独自に提案してください（「独自提案」と注記）。

### 1. 標準訳
原文に忠実な直訳寄りの英訳。文法的に正確で確実な表現を選択。
学習者が参考にできるレベル。

### 2. 再構成訳
文構造を英語的に再編成。情報の順序・接続表現・主語の選択を最適化。
より洗練された英語を目指す場合に「こう書くとより良い」と示すレベル。
...
### ベスト英訳
各LLMの英訳から最も優れた表現を組み合わせて、1つの最善英訳を合成。
英文としての完成度を最優先（正確性・自然さ・表現力）。
各LLMの英訳にない表現でもより適切なものがあれば、独自に採用してよい。

### 表現注釈
ベスト英訳で採用した表現について、以下を注釈として付記:
- 採用元のLLM名（独自提案の場合は「独自提案」）
- 他LLMの代替表現（使える場合）
- 語彙・構文の選択理由（簡潔に）
...
### 各LLMの英訳一覧
LLMごとに訳A・訳Bをそのまま提示（原文のまま、編集しない）。
...
### 注目すべき表現差異
各LLMの英訳で特に差が出た箇所をピックアップ。
それぞれの表現の長所・短所を比較。

### 英文品質ランキング
1位〜4位を、正確性・自然さ・表現力の観点から理由とともに提示。

### 独自の改善提案
各LLMの英訳を踏まえ、さらに良い表現がないか検討。改善案があれば提示。"""
