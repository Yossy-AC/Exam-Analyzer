...
- **分析画面**: 7タブ構成（ダッシュボード / 長文統計 / 英作文統計 / 問題形式選択 / 類似検索 / データ一覧 / その他）。モバイルは4タブに統合
- **大学属性フィルター**: 大学分類・地域による折りたたみフィルターパネル（問題形式・類似検索・データ一覧・管理画面）
...
- **Multi-LLM英訳ツール**: 4種LLM（Claude, Gemini, ChatGPT, Grok）を並列呼び出しし、Claude統合で模範解答を生成。英訳レビュー・減点シミュレーション・大学別チューニング対応。履歴閲覧可能
- **認証**: パスワードベースの簡易認証（オプション）。ポータル経由時はstudentロール（`student`/`student_exam`/`student_files`）の書き込み・エクスポート制限あり
